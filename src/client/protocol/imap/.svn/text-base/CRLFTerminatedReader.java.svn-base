...
public class CRLFTerminatedReader extends Reader {
	public static int DEFAULT_BUFF_SIZE = 8*1024;
	...
	public String readLine() throws IOException {
		...
		// Until we add support for specifying a maximum line length as
		// a Service Extension, limit lines to 2K, which is twice what
		// RFC 2821 4.5.3.1 requires.
		while (lineBuffer.length() <= DEFAULT_BUFF_SIZE) {
			int inChar = read();
	...
	public int readNext(byte bbuf[], int len) throws IOException {
		int result = 0;
		while(result < len) {
			int ch = in.read();
			if(ch == -1) {
